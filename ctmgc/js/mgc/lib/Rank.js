mgc.lib.Rank = CT.Class({
	CLASSNAME: "mgc.lib.Rank",
	_available: 0,
	_string: "",
	_cards: [],
	_ranks: {
		...
		"KIND": function(num) {
			...
			for (hasValue in _.values) {
				if (num == 1)
					kinds.push(new mgc.lib.Rank([_.values[hasValue][0]]));
				else if (_.values[hasValue].length == num)
					kinds.push(new mgc.lib.Rank(_.values[hasValue]));
			}
		...
		"HOUSE": function() {
			...
			rank._is("KIND", [3], function(matches) {
				matches.forEach(function(three) {
					...
					newRank = new mgc.lib.Rank(remaining);
					newRank._is("KIND", [2], function(pmatches) {
							fullHouse =
								new mgc.lib.Rank(three._cards.concat(pmatches[0]._cards));
		...
		"FLUSH": function() {
			...
			for (var hasSuit in _.suits) {
				...
				if (flush.length > 4) {
					...
					flushs = [new mgc.lib.Rank(flush.slice(0, 5))];
				}
			}
			return flushs;
		},
		"STRAIGHT": function() {
			var rank = this, index = 0, straight, newStraight,
				...
				findStraights  = function(low) {
					...
					straights.forEach(function(s) {
						if (s.length == 5) {
							returnStraights.push(new mgc.lib.Rank(s));
						}
					});
				};
			while (index < 3) {
		...
		"2PAIR": function() {
			...
			rank._is("KIND", [2], function(matches) {
				if (matches.length > 1) {
					...
					pairses.push(new mgc.lib.Rank(pairs));
				}
			})
			return pairses;
		}
	...
	"_sub": function(matched) {
		var reducedCards = this._filter(matched),
			newRank = new mgc.lib.Rank(reducedCards, this._available);
		newRank._fill();
		this._available = 0;
...
mgc.lib.Rank._test = function() {
	...
	*/
	// FLUSHAKQ53 FROM 7
	TEST = new mgc.lib.Rank([
			new mgc.lib.Card("heart", "4"),
			new mgc.lib.Card("spade", "5"),
			new mgc.lib.Card("spade", "Q"),
			new mgc.lib.Card("spade", "K"),
			new mgc.lib.Card("spade", "2"),
			new mgc.lib.Card("spade", "3"),
			new mgc.lib.Card("spade", "A")
		], 5);
	TEST._best();
	console.log(TEST._string);

	// FLUSHKQ532 FROM 7
	TEST2 = new mgc.lib.Rank([
			new mgc.lib.Card("heart", "4"),
			new mgc.lib.Card("spade", "5"),
			new mgc.lib.Card("spade", "Q"),
			new mgc.lib.Card("spade", "K"),
